...
class Timer(object):
	"""This is a timer object. D-uh"""
	def __init__(self):
		self.global_start = time.time()
		self.between_start = self.global_start

	def timer(self, label):
		"""This is the timer function"""
		now = time.time()
		sys.stderr.write("%s: total_time=%.7fs between_time=%.7fs\n" % (label, now-self.global_start, now-self.between_start))
		self.between_start = now

def rrelu(training, low, high):
	"""This is my rrelu GENERATOR function"""
...
def process_images(data, q):
	"""this is the process image loop"""
	filenames, dog_predicates = zip(*data)
	actualsz = len(data)
	np_input_images = np.zeros((actualsz, 256, 256, 3))
	np_dog_predicates = np.array(dog_predicates).reshape(len(data), 1)
	for idx in xrange(0, actualsz):
		...
		resized = rotated.resize((256, 256), resample=Image.BICUBIC)
		np_input_images[idx] = (np.array(resized) / 255.0) * 2.0 - 1.0
        print "put block to queue"
	q.put((np_input_images, np_dog_predicates, actualsz))
	return None
#pylint: enable=invalid-name


#pylint: disable=not-context-manager,too-many-locals,too-many-statements,invalid-name
def main():
	"""This is the main"""
...
#			get_from_buffer = tf.identity(var_buffer_out)
		...
		while True:
			pool = multiprocessing.Pool(processes=4)
                        gpupool = multiprocessing.Pool(processes=4)
			m = multiprocessing.Manager()
			q = m.Queue()
			epoch_start = time.time()
			epoch += 1
			# one epoch.
			random.shuffle(info["train"])
			for start in xrange(0, len(info["train"]), batchsz):
				end = min(start + batchsz, len(info["train"]))
				data = info["train"][start:end]
				(ignored_variable) = pool.apply_async(process_images, args=(data, q, ))
			trainloss = 0
#			for start in xrange(0, len(info["train"]), batchsz):
#				end = min(start+batchsz, len(info["train"]))
#				actualsz = end - start
			timer.timer("about to wait for queue")
                        pool.close()
                        pool.join()
			grabbed = 0
			while not q.empty():
				
				(np_input_images, np_dog_predicates, actualsz) = q.get()
				timer.timer("got something from q")
				batchloss, _ = sess.run([loss, step], feed_dict={input_images_: np_input_images, dog_predicates_: np_dog_predicates, training_: True})
				timer.timer("sess.run end %d " % epoch)
				trainloss += batchloss * actualsz
				grabbed += 1
				sys.stderr.write("grabbed: %d\n" % (grabbed))
			trainloss = trainloss / len(info["train"])

			# Now, let's do the validation.
			validationloss = 0
			valcopy = copy.deepcopy(info["validation"])
			for start in xrange(0, len(info["validation"]), batchsz):
				if epoch == 1:
					sys.stderr.write("validation epoch: %d start=%d %d\n" % (epoch, start, len(info["validation"])))
				end = min(start + batchsz, len(info["validation"]))
				data = info["validation"][start:end]
				filenames, dog_predicates = zip(*data)
				actualsz = len(data)
				np_input_images = np.zeros((actualsz, 256, 256, 3))
				np_dog_predicates = np.array(dog_predicates).reshape(len(data), 1)
				...
				validationloss += batchloss * actualsz
			with open("./dogs/logs/%08d.json" % (epoch), "w") as logfp:
				json.dump(valcopy, logfp, indent=4, sort_keys=True)
			validationloss = validationloss / len(info["validation"])
